...
public class NoteController {
    ...
    public ResponseEntity<Response> getAllArchiveNotes(@RequestHeader String token) {
        List<NoteModel> noteModel = noteService.getAllArchiveNotes(token);
        Response response = new Response(200, "all notes fetch successfully", noteModel);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
    ...
    public ResponseEntity<Response> getAllPinNotes(@RequestHeader String token) {
        List<NoteModel> noteModel = noteService.getAllPinNotes(token);
        Response response = new Response(200, "all notes fetch successfully", noteModel);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
    ...
    public ResponseEntity<Response> getAllTrashNotes(@RequestHeader String token) {
        List<NoteModel> noteModel = noteService.getAllTrashNotes(token);
        Response response = new Response(200, "all notes fetch successfully", noteModel);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
    ...
    public ResponseEntity<Response> addCollaborator(@RequestParam String emailId, @PathVariable String noteId, @RequestParam String collaborator, @RequestHeader String token, @PathVariable Long collabaratorId) {
        Response response = noteService.addCollaborator(emailId, noteId, Long.valueOf(collaborator), token);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
